<script lang="ts">
	type IconType = 'edit' | 'delete' | 'add' | 'chevron-up' | 'chevron-down' | 'close' | 'back' | 'menu' | 'check' | 'refresh';
...
{#if href && !disabled}
	<a
		{href}
		class="inline-flex rounded-lg transition-colors focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-1 {variantStyles[variant]} {sizes.button} {className}"
		aria-label={label}
		title={label}
	>
		{@render iconContent()}
...
{:else}
	<button
		type="button"
		class="rounded-lg transition-colors focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-1 disabled:opacity-50 disabled:cursor-not-allowed {variantStyles[variant]} {sizes.button} {className}"
		{disabled}
		{onclick}
		aria-label={label}
		title={label}
